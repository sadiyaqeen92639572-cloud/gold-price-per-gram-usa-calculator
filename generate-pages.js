...
<div class="content" style="padding-top:64px;">
  ...
  <p>This site's live gold price does not come from a US-dollar spot feed directly. It comes from our sister site's UK-based provider (<a href="https://www.goldapi.io/" target="_blank" rel="noopener">goldapi.io</a>, queried in GBP), converted to USD using a live GBP→USD exchange rate from <a href="https://frankfurter.dev/" target="_blank" rel="noopener">frankfurter.dev</a> (European Central Bank reference rates, no API key required). We disclose this rather than presenting the USD figure as if it came from a native dollar feed — it's a two-step calculation, and the accuracy of the USD price depends on both the gold spot price and the FX rate being current.</p>
